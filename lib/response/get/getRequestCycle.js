...
module.exports = function getRequestCycle(getResponse, model, results, observer,
    ...
    if (count === model._maxRetries) {
        retryErr = new MaxRetryExceededError();
        if (model._eventLog) {
            model._eventLog(
                "GetResponse", "onError", getResponse, [retryErr]
            );
        }
        observer.onError(retryErr);
        return {
            dispose: function() {}
        };
    }
    ...
    var currentRequestDisposable = requestQueue.
        get(boundRequestedMissingPaths, optimizedMissingPaths, function(err) {
            ...
            if (err instanceof InvalidSourceError) {
                if (model._eventLog) {
                    model._eventLog(
                        "GetResponse", "onError", getResponse, [err]
                    );
                }
            ...
            // we can.
            var nextResults = checkCacheAndReport(getResponse, model,
